...
{
$sql ="SELECT * FROM  alumni_login WHERE username='".$user_name."' AND passwd='".$current_password."'";
$result=mysqli_query($conn, $sql);

if($login=mysqli_fetch_assoc($result))
{
	$sql1= "UPDATE alumni_login SET passwd='".$new_password."' WHERE username='".$user_name."' and Passwd='".$current_password."'";
...
if (mysqli_query($conn, $sql)) {
     echo "<script>alert(' password saved successfully!!!!');</script>";
	  header("refresh:0; url=alchpass.html");
} else {
